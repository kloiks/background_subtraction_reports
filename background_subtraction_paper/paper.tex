...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\columnwidth]{figs/Fig1_synthetic_data_variations.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{figs/Fig6_visual_eq_Weikang_4.png}
...
We first demonstrate convergence and consistency of the outputs of the optimization framework. Then, we present a quantitative comparison of the optimized methods across datasets, followed by analysis of optimized parameters and representative visual examples. Finally, we evaluate the iterative two-stage refinement described in Section~2.6 -- our combined approach for whole-pattern background subtraction, including the equatorial streak -- using the same standardized metrics, reported separately for the equatorial region, the general region, and the full pattern.

% Next, we evaluate it against a set of human expert labelled patterns to assess whether the proposed framework improves upon existing approach and corresponds to perceptually meaningful background subtraction quality.

\subsection{Convergence and Consistency of Optimization}

The optimization framework demonstrates robust convergence and consistency across different datasets. Figure~\ref{fig:consistency_ranking} reflects low average loss dispersion within datasets for all but two datasets, which suggests the need for image-subset tuning. It also shows that although the best-performing method varies with dataset, WTH is ranked first in 6 of the 11 datasets, with the remaining datasets split among G-ILPF, RW, and AVG. Figure ~\ref{fig:consistency_bgsum} shows the average background intensity per dataset, which is consistent within datasets and varies more in the datasets that contain patterns taken at different exposure times, where the intensity is expected to vary with exposure time.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{figs/Fig7_consistency_ranking.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{figs/Fig8_consistency_bgsum.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{figs/Fig9_visual_no_eq_Jani_21.png}
\hspace{0.01\linewidth}
\includegraphics[width=0.8\linewidth]{figs/Fig9_visual_no_eq_MPLL_20.png}
\hspace{0.01\linewidth}
\includegraphics[width=0.8\linewidth]{figs/Fig9_visual_no_eq_Granzier_21.png}
...
% \begin{figure}[htbp]
% \centering
% \includegraphics[width=0.9\columnwidth]{figs/Fig10_expert_comparison.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{figs/Fig11_visual_eq_Weikang_4.png}
\hspace{0.01\linewidth}
\includegraphics[width=0.8\linewidth]{figs/Fig11_visual_eq_Jani_21.png}
\hspace{0.01\linewidth}
\includegraphics[width=0.8\linewidth]{figs/Fig11_visual_eq_MPLL_2.png}
\hspace{0.01\linewidth}
\includegraphics[width=0.8\linewidth]{figs/Fig11_visual_eq_Granzier_4.png}
...
% Taken together, these results demonstrate that the proposed optimization framework provides a consistent and objective basis for comparing background subtraction methods across diverse datasets. The framework identifies methods that perform well on average while also revealing substantial variability in optimal parameter values.

% This variability motivates future work on dataset-specific or image-specific parameter selection strategies. Importantly, the framework is intended to guide parameter selection and method adaptation, rather than to prescribe a single fixed algorithm or parameter set. 

Taken together, these results demonstrate that the proposed optimization framework provides a consistent and objective basis for comparing background subtraction methods across diverse datasets. Combined with the iterative fitting (Section~2.6), the framework allows background subtraction across the whole pattern, a significant improvement over existing approaches that typically work only on specific regions of the pattern. This separation of background components will enable more accurate analysis of diffraction features across the entire pattern and contribute to future studies of how background structure changes across experimental conditions, since the background itself may contain useful information.

% \paragraph{Limitations and Recommended Approach}
The appropriate choice of background subtraction strategy still depends on the data analysis goal: for low angles only, the RCH method is often efficient, sufficient and robust; for layer lines and high angles, the equator may be masked and the optimization framework applied to the rest of the pattern; for background subtraction across the whole pattern, the iterative combined approach with equatorial and general background fitting may be used.
...
% 
% Across the eleven datasets evaluated, the white top-hat filter (WTH) achieved the lowest average loss and ranked first in six of the eleven datasets, with G-ILPF, RW, and AVG taking the remaining top ranks (Section~3.1). 
...
Combining the iterative two-stage equatorial-general fit with a non-parametric framework method extends background subtraction across the whole pattern, including the equatorial streak, and improved on the RCH baseline in both the general-background region and the equatorial region itself. The parametric fit alone removed the dominant, anisotropic equatorial streak but left a high residual baseline; applying a non-parametric framework method to that residual removed the remaining slowly varying component and gave the lowest loss of any method or combination tested in both regions. Parametric and non-parametric approaches therefore address different failure modes and are most effective combined rather than used in isolation.

Two limitations constrain how broadly these results generalize. Optimized parameters vary substantially across datasets, and to a lesser extent within them, so a single globally tuned parameter set is unlikely to be optimal; dataset- or image-specific tuning should be applied rather than fixed defaults. The loss weights also require operator judgment: the framework automates the parameter search within a method but not the choice of how strongly to penalize each failure mode for a given task. We recommend a hybrid workflow -- automated search combined with human verification on a small image subset, followed by weight adjustment and full-dataset optimization -- rather than a fully unsupervised pipeline.
