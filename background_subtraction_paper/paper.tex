...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\columnwidth]{figs/Fig1_synthetic_data_variations_v2.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{figs/Fig4_ch_example_Jani_1_zoomed.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{figs/Fig6_visual_eq_Weikang_4_half.png}
...
We first demonstrate convergence and consistency of the outputs of the optimization framework. Then, we present a quantitative comparison of the optimized methods across datasets, followed by analysis of optimized parameters and representative visual examples. Finally, we evaluate the iterative two-stage refinement described in Section~\ref{sec:two_stage} -- our combined approach for whole-pattern background subtraction, including the equatorial streak -- using the same standardized metrics, reported separately for the equatorial region, the general region, and the full pattern.
...
The optimization framework demonstrates robust convergence and consistency across different datasets. Figure~\ref{fig:consistency_ranking} reflects low average loss dispersion within datasets for all but three datasets, which suggests the need for image-subset tuning. It also shows that although the best-performing method varies with dataset, WTH is ranked first in 6 of the 11 datasets, with the remaining datasets split among G-ILPF, RW, and AVG. Figure ~\ref{fig:consistency_bgsum} shows the average background intensity per dataset, which is consistent within datasets, which contain pattern taken with the same exposure time. Optimization traces reflect that the optimization process is stable and makes choices based on the assigned weights. All the methods converge to similar loss values and the stopping criteria is either no loss improvement or very small change.

%  and varies more in the datasets that contain patterns taken at different exposure times, where the intensity is expected to vary with exposure time.


\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{figs/Fig7_consistency_ranking.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{figs/Fig8_consistency_bgsum.png}
...
\hspace{0.01\linewidth}
\includegraphics[width=0.8\linewidth]{figs/Fig9_visual_no_eq_sg_Granzier_21.png}
...
The iterative two-stage pipeline described in Section~\ref{sec:two_stage} produces a single residual, $\mathrm{img} - E - G$, covering the entire pattern, including both the equatorial streak and the surrounding general background. We evaluate the performance of this approach by analyzing the residual in both the general high-angle region (same as in the previous section) and the equatorial region. Reporting a single full-pattern number would obscure how the two fitted components contribute to the result in each region, since the low angle equatorial and high angle general regions require different masking. For the equatorial region evaluation, the aggregate loss is computed with a modified set of weights relative to Section~\ref{sec:optimization_procedure}: the smoothness term is dropped ($\lambda_1 = 0$), since in the equatorial region, the background is expected to be steep. Relative to the default weighting used there ($[\lambda_1,\ldots,\lambda_5] = [0.1, 0.05, 0.05, 0.01, 0.8]$), the weight removed from smoothness is redistributed evenly to the two oversubtraction terms, $f_{\mathrm{neg,con}}$ and $f_{\mathrm{neg,syn}}$, while $\mathrm{NMSE}_{\mathrm{syn}}$ and $1-f_B$ keep their default weights, giving $[\lambda_1, \lambda_2, \lambda_3, \lambda_4, \lambda_5] = [0, 0.1, 0.1, 0.01, 0.8]$. The evaluation of the general background region is performed in the same way as in the previous section.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{figs/Fig11_visual_eq_Weikang_4_v2.png}
\hspace{0.01\linewidth}
\includegraphics[width=0.8\linewidth]{figs/Fig11_visual_eq_Jani_21_v2.png}
\hspace{0.01\linewidth}
\includegraphics[width=0.8\linewidth]{figs/Fig11_visual_eq_MPLL_2.png}
\hspace{0.01\linewidth}
\includegraphics[width=0.8\linewidth]{figs/Fig11_visual_eq_Granzier_4_v2.png}
...
% 
% Across the eleven datasets evaluated, the white top-hat filter (WTH) achieved the lowest average loss and ranked first in six of the eleven datasets, with G-ILPF, RW, and AVG taking the remaining top ranks (Section~\ref{sec:convergence}).
